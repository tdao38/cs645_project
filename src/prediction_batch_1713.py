...
if __name__ == '__main__':
    ...
    predicted_interval['label'] = np.where(predicted_interval['ratio'] >= 0.2, 1, 0)
    predicted_interval[['start', 'end', 'label']].to_csv('prediction_result/exstream/batch146_13_predicted_exstream.csv', index=False)
    print(predicted_interval)

    print('Machine learning model')
    predicted_interval_ml = predict_interval(predicted_data_ml, test_interval)
    predicted_interval_ml['label'] = np.where(predicted_interval_ml['ratio'] >= 0.2, 1, 0)
    predicted_interval_ml[['start', 'end', 'label']].to_csv('prediction_result/extension/batch146_13_predicted_ml.csv', index=False)
